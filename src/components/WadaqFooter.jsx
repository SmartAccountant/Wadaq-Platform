...
const MODAL_TITLES = {
  ...
  refund: "سياسة الإرجاع والاستبدال",
  contact: "اتصل بنا",
};

/**
 * تذييل بألوان الهوية (أزرق داكن + حافة ذهبية)، روابط قانونية تفتح نوافذ منبثقة
 */
export default function WadaqFooter() {
  ...
  const quickLinks = [
    ...
    { id: "refund", label: "سياسة الإرجاع والاستبدال" },
    { id: "contact", label: "اتصل بنا" },
  ];

  const linkBtnClass =
    "block w-full text-right text-slate-200/95 hover:text-white hover:underline py-1 text-sm transition-colors";
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-8 lg:gap-12">
            {/* عمود التواصل — يمين الشاشة في RTL */}
            <div className="space-y-4">
              <h3 className="font-bold text-lg text-white">مؤسسة ثروة لتقنية المعلومات</h3>
              <ul className="space-y-3 text-sm text-slate-300/95 leading-relaxed">
                <li className="flex gap-3 items-start">
                  ...
                  <span>الرياض — حي السفارات، المملكة العربية السعودية</span>
                </li>
                <li className="flex gap-3 items-start">
            ...
            <div>
              ...
              <ul className="space-y-1">
                ...
                <li className="pt-2 border-t border-white/10 mt-2">
                  <Link
                    to="/checkout"
                    className="block w-full text-right text-amber-200/90 hover:text-amber-100 text-sm font-semibold py-1"
                  >
                    بوابة الدفع
                  </Link>
                </li>
            ...
            {/* طرق الدفع */}
            <div>
              <h4 className="font-bold mb-4 text-white">طرق الدفع المتاحة</h4>
              <div className="flex flex-wrap gap-3 items-stretch justify-start">
                ...
                >
                  ...
                  <span className="text-[10px] font-bold text-slate-800 pr-0.5">Mastercard</span>
                </div>
                <div
              ...
              <p className="mt-4 flex items-start gap-2 text-xs text-slate-400/95 leading-relaxed">
                ...
                <span>جميع المعاملات محمية بأعلى معايير الأمان</span>
              </p>
            </div>
          </div>

          <div className="mt-10 pt-6 border-t border-white/15 text-center text-xs text-slate-400">
            © {new Date().getFullYear()} مؤسسة ثروة لتقنية المعلومات — جميع الحقوق محفوظة ·{" "}
            <a href="https://rikazai.com" className="hover:text-slate-200 underline-offset-2 hover:underline" target="_blank" rel="noopener noreferrer">
              rikazai.com
            </a>
          </div>
        </div>
      ...
      <Dialog open={modal != null} onOpenChange={(open) => !open && setModal(null)}>
        <DialogContent
          className="max-w-4xl w-[min(100vw-1rem,56rem)] max-h-[min(90vh,880px)] overflow-hidden flex flex-col p-0 gap-0 border-slate-700 bg-slate-950 sm:rounded-xl [&>button]:left-4 [&>button]:right-auto"
        >
          <DialogTitle className="sr-only">{modal ? MODAL_TITLES[modal] : ""}</DialogTitle>
          <div className="overflow-y-auto flex-1 min-h-0">
            ...
            {modal === "contact" && (
              <div className="p-6 sm:p-8 text-right text-slate-200 space-y-6 bg-slate-900" dir="rtl">
                <h2 className="text-xl font-bold text-white">اتصل بنا</h2>
                <p className="text-slate-300 text-sm leading-relaxed">
                  نسعد بتواصلكم عبر البريد أو الهاتف أو زيارة العنوان أدناه.
                </p>
